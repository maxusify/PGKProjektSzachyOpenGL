...
void Pawn::computeAvailableMovements(
    std::vector<Piece*> own,
    std::vector<Piece*> opp
) {
    ...
    if (!found && pos[0]>=0 && pos[0]<8 && pos[1]>=0 && pos[1]<8) 
    {
        ...
        if ((whichSide==-1 && posX==6)||(whichSide==1 && posX==1)) 
        {
            pos[0] = posX+2*whichSide;
            pos[1] = posY;
